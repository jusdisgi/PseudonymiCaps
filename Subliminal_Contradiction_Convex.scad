...
/* ============================================================================
   SUBLIMINAL CONTRADICTION - CONVEX (thumb / mod caps)
   Convex domed companions to Subliminal_Contradiction.scad. Reconstructed in
   the Pseudonymicaps convex engine (DishShape bulge) for Choc / PG1316S mounts
   at arbitrary spacing. Sizes 1.0-2.25u plus a slanted 1u thumb.
   keyID: 0 slanted-1u | 1 flat-1u | 2:1.25u 3:1.5u 4:1.75u 5:2.0u 6:2.25u
   ============================================================================ */

// ----------------------------- USER KNOBS -----------------------------------
keyID  = 1;            // 0 slanted 1u, 1 flat 1u, 2..6 = 1.25/1.5/1.75/2.0/2.25u
unit_x = 17;           // key pitch X (16 / 17 / 18 / 19.05 ...)
unit_y = 17;           // key pitch Y
...
//uW,  uL, TWDif, TLDif, keyh, WSft, LSft, XSkew, YSkew, ZSkew, WEx, LEx, CapR0i, CapR0f, CapR1i, CapR1f, CapREx, StemEx
  [1.00, 1, 5.6, 5.0, 4.8, 0, -0.5,  7, 0, 0, 2, 2.0, .10, 3, .10, 3, 2, 2], //0 convex slanted 1u (thumb)
  [1.00, 1, 5.6, 5.0, 4.8, 0,  0.0,  0, 0, 0, 2, 2.0, .10, 3, .10, 3, 2, 2], //1 convex flat 1u
  [1.25, 1, 5.6, 5.6, 4.8, 0,  0.0,  0, 0, 0, 2, 2.5, .10, 3, .10, 3, 2, 2], //2 convex 1.25u
  [1.50, 1, 5.6, 5.6, 4.8, 0,  0.0,  0, 0, 0, 2, 2.5, .10, 3, .10, 3, 2, 2], //3 convex 1.5u
  [1.75, 1, 5.6, 5.6, 4.8, 0,  0.0,  0, 0, 0, 2, 2.5, .10, 3, .10, 3, 2, 2], //4 convex 1.75u
  [2.00, 1, 5.6, 5.6, 4.8, 0,  0.0,  0, 0, 0, 2, 2.5, .10, 3, .10, 3, 2, 2], //5 convex 2.0u
  [2.25, 1, 5.6, 5.6, 4.8, 0,  0.0,  0, 0, 0, 2, 2.5, .10, 3, .10, 3, 2, 2], //6 convex 2.25u
];
...
function BottomWidth(keyID)  = unit_x*keyParameters[keyID][0] - gap;  //col0 = width units
  ...
  difference(){
    difference(){
      union(){
        ...
        if(Stem == true){
          translate([0,0,StemBrimDep])rotate([0,0,StemRot]){ if(stemType=="MX") mx_stem(); else choc_stem(); }
          if (Stab != 0){
...
module mx_stem(stemH = 3.6, slop = 0.0) {  // basic Cherry MX cross stem (approx)
  translate([0,0,-stemH/2 + stemCrossHeight])
  difference() {
    cylinder(h=stemH, d=5.5+slop, center=true, $fn=48);
